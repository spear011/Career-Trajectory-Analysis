...
Constructs temporal occupation transition networks
"""
...
def prepare_job_data(job_df):
    ...
    job_df = job_df[job_df[NETWORK_OCCUPATION_COLUMN] != '99-9999.00'].copy()
    
    # Select required columns only
    cols = ['ID', NETWORK_OCCUPATION_COLUMN, NETWORK_OCCUPATION_NAME_COLUMN, 
    ...
    print(f"  Unique occupations: {job_df[NETWORK_OCCUPATION_COLUMN].nunique():,}")
    ...
    for user_id, group in job_df_sorted.groupby('ID'):
        for _, row in group.iterrows():
            user_paths[user_id].append({
                'occupation': row[NETWORK_OCCUPATION_COLUMN],
                'occupation_name': row[NETWORK_OCCUPATION_NAME_COLUMN],
...
def build_temporal_network(transitions_df, wage_df, period='annual'):
    ...
    for year in sorted(transitions_df['transition_year'].unique()):
        ...
        # Add nodes: all occupations appearing in this year
        ...
        for occ in all_occupations:
            ...
            # Calculate employment count (people who transitioned from/to this occupation in this year)
...
def save_networks(networks, stats_df, output_dir):
    """Save networks"""
    print("\nSaving networks...")
    ...
    # 1. Save each year's network as GraphML
    for year, G in networks.items():
        # Skip NaN years
        if pd.isna(year):
            continue
            
        # Convert None values to string for GraphML compatibility
        G_copy = G.copy()
    ...
    # 2. Save all networks as pickle (for Python analysis)
    ...
    # 4. Save metadata
    metadata = {
        'occupation_column': NETWORK_OCCUPATION_COLUMN,
        'occupation_name_column': NETWORK_OCCUPATION_NAME_COLUMN,
